...
var endpointGroups = {
	...
	index : _(endpoints).wherePropContainsAll('path', ['index']).groupBy('op').value(),
	type : _(endpoints).wherePropContainsAll('path', ['index', 'type']).groupBy('op').value(),
	doc : _(endpoints).wherePropContainsAll('path', ['index', 'type', 'id']).groupBy('op').value()
};
